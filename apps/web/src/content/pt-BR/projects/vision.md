...
Vision integra perfeitamente com seu stack existente através de nosso ecossistema crescente de integrações de framework, conectores de camada de dados e exportadores.

**Frameworks Web**: Express.js, Fastify, Koa, NestJS
**Camada de Dados**: Prisma ORM
**Exportadores**: Datadog (com conformidade OpenTelemetry)

Navegue pelo ecossistema completo de integrações usando o menu de navegação ao visualizar qualquer página de integração. Cada integração inclui exemplos abrangentes, variantes de configuração e padrões de implantação de produção.
...
Pronto para adicionar observabilidade estruturada à sua aplicação?

1. Instale o núcleo do Vision: `npm install @rodrigopsasaki/vision`
2. Escolha sua integração de framework no menu do ecossistema
3. Siga o guia de configuração abrangente para seu framework específico

Cada integração oferece configuração zero, múltiplos padrões de implantação e exemplos prontos para produção.
